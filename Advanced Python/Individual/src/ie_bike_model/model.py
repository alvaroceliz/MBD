...
def load_process_training_data():
    """
    Retrieves the data (hour.csv) located in a file called data and creates feature engineering and transformation of variables to prepare it for 
    training
    """

    # Data Preparation
    actual_dir, _ = os.path.split(__file__)

    DATA_PATH = os.path.join(actual_dir + "/data")

    df_bike = pd.read_csv(DATA_PATH + "/hour.csv", index_col="instant", parse_dates=True)

    # Change variable dteday to date time
    df_bike["dteday"] = pd.to_datetime(df_bike["dteday"])

    # Resolving skewness with log and square root
    df_bike["windspeed"] = np.log1p(df_bike["windspeed"])
    df_bike["cnt"] = np.sqrt(df_bike["cnt"])

    # FEATURE ENGINEERING
    # Binning temp, hum in 5 equally sized bins
    bins = [0, 0.19, 0.49, 0.69, 0.89, 1]
    df_bike["temp_binned"] = pd.cut(df_bike["temp"], bins).astype("category")
    df_bike["hum_binned"] = pd.cut(df_bike["hum"], bins).astype("category")
    ...
    # Drop of some irrelevant variables
    df_bike = df_bike.drop(columns=["dteday", "atemp", "casual", "registered"])

    return df_bike

# Train and Persist function
def train_and_persist(persist=None, random_state=42, compression_factor=False):
    """
    Training a RandomForestRegressor model and persist it as a pkl object in the user's
    ...
    # Creation of the target variable (cnt)
